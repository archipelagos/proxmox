...
Download https://www.proxmox.com/en/downloads/proxmox-virtual-environment/iso/proxmox-ve-8-1-iso-installer[Proxmox Virtual Environment installer].
Use `dd` command like presented below to populate USB Flash Drive.
...
Remember to use raw block device like `/dev/sda`, `/dev/sdb` (or other) instead
of partition block device like `/dev/sda1`, `/dev/sdb1` (or other).
