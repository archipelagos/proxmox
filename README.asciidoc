...
load-module module-native-protocol-tcp auth-ip-acl=192.168.4.0/24 auth-anonymous=1
-------------------------------------------------------------------------------

Start server as *current desktop user*. In case of non-desktop users just make
sure that user belongs to proper groups if something goes wrong.
...
=== Server configuration with access restriction

Server can restrict connections for clients who are not able to authenticate.
To achieve that you need to modify `/etc/pulse/default.pa` file again. Your
line should not contain ` auth-anonymous=1` this time.
...
This means that each client in the network need to authenticate to establish
connection. Without that client is not able to stream audio - server will just
ignore it. Token based authentication is used for valid connection. Both sides
need to share the same token, which is called cookie.
