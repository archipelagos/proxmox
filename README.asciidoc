...
Add following line to `/etc/pulse/default.pa` file. Populate it. For start do
not worry about any access restrictions. For base setup each host is allowed to
connect to server and stream its own audio.

[subs="+attributes"]
-------------------------------------------------------------------------------
[{local_user}@{local_hostname} ~]$ tail -n 1 /etc/pulse/default.pa
load-module module-native-protocol-tcp auth-ip-acl=127.0.0.1;192.168.4.0/24 auth-anonymous=1
-------------------------------------------------------------------------------
...
=== Server configuration with ACL based access
...
* https://wiki.archlinux.org/title/PulseAudio/Examples#Set_default_input_source[PulseAudio/Examples]
